...
static void dump_reg(struct cpld_data *data)
{
    ...
    dev_info(&client->dev, "[PORT_PLD2]PORT_RESET_REG: 0x%02x, 0x%02x\n", val0, val1);

    val0 = cpld_i2c_read(data, PORT_LPMODE_REG0);
    val1 = cpld_i2c_read(data, PORT_LPMODE_REG0 + 1);
    dev_info(&client->dev, "[PORT_PLD2]PORT_LPMODE_REG: 0x%02x, 0x%02x\n", val0, val1);

    val0 = cpld_i2c_read(data, PORT_MODPRS_REG0);
    val1 = cpld_i2c_read(data, PORT_MODPRS_REG0 + 1);
    dev_info(&client->dev, "[PORT_PLD2]PORT_MODPRES_REG: 0x%02x, 0x%02x\n", val0, val1);
...
static const struct attribute_group port_pld2_group = {
    .attrs = port_pld2_attributes,
};

static int port_pld2_probe(struct i2c_client *client)
{
    int status;
    ...
    status = sysfs_create_group(&client->dev.kobj, &port_pld2_group);
    if (status) {
...
static void port_pld2_remove(struct i2c_client *client)
{
    struct cpld_data *data = i2c_get_clientdata(client);
    sysfs_remove_group(&client->dev.kobj, &port_pld2_group);
    kfree(data);
}

static const struct of_device_id port_pld2_of_ids[] = {
    {
        .compatible = "port_pld2",
        .data       = (void *) 0,
    },
    { },
};
MODULE_DEVICE_TABLE(of, port_pld2_of_ids);

static const struct i2c_device_id port_pld2_ids[] = {
    { DRIVER_NAME, 0 },
    { }
};
MODULE_DEVICE_TABLE(i2c, port_pld2_ids);

static struct i2c_driver port_pld2_driver = {
    .driver = {
        .name           = DRIVER_NAME,
        .of_match_table = of_match_ptr(port_pld2_of_ids),
    },
    .probe        = port_pld2_probe,
    .remove       = port_pld2_remove,
    .id_table     = port_pld2_ids,
    .address_list = cpld_address_list,
};

static int __init port_pld2_init(void)
{
    return i2c_add_driver(&port_pld2_driver);
}

static void __exit port_pld2_exit(void)
{
    i2c_del_driver(&port_pld2_driver);
}

MODULE_AUTHOR("Nokia");
MODULE_DESCRIPTION("NOKIA H6-128 PORT_PLD2 driver");
MODULE_LICENSE("GPL");

module_init(port_pld2_init);
module_exit(port_pld2_exit);
